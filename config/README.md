...
`game-prototype-v7` はアルゴリズム版、`match-config-v1` は設定構造の版です。**係数だけの変更は保存される設定内容で識別**します。式・分岐・設定項目・許容範囲を変える場合は新しい版を追加し、旧版の再実行を維持してください。固定の検査範囲は [config-schema.ts](../src/game/config-schema.ts)、各範囲は `min`（下限）・`max`（上限）、配列は `items`（要素の範囲）・`length`（要素数）で表し、日本語コメントを付けています。通常の係数調整ではスキーマを変更せず、`match.ts`を編集してください。項目間の条件は [config.ts](../src/game/config.ts) です。検査範囲は数値破綻防止の制約で、校正済みの推奨範囲ではありません。
