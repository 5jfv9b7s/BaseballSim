...
export type ConfiguredPitchStep = Omit<PitchStep, 'state'> & {
  state: PitchContext;
...
export function simulateConfiguredPitch(
  input: PitchContext,
  fixture: Fixture,
  commandId: string,
  config: MatchConfig['pitch'],
): ConfiguredPitchStep {
  const m = {
    ...config,
    version: GAME_MODEL_V7.pitchModelVersion,
  ...
  const swingProbability = swingChance(inside, aggression, discipline, input.count, m);
  ...
  if (action === 'take') {
    ...
    const ability =
      (pitcher.throwingHand === 'R' ? batter.batting.contactVsRight : batter.batting.contactVsLeft)
        .valueMilli / 120000;
    contactProbability = contactChance(inside, ability, velocityCentiKph, input.count.strikes, m);
  ...
  const step: ConfiguredPitchStep = {
    state,
    decision: {
...
/** abilityはRatingMilli/120000、球速は0.01km/h。補正は2ストライク時だけ。 */
export function contactChance(
  ...
  const protection = strikes === 2 ? m.contactProtectBase + m.contactProtectAbility * ability : 0;
