...
import { PITCH_MODEL_V3 as m } from './model-v3.ts';
import { avoidBodyContact } from './pitch-v2.ts';
...
export type PitchV3Step = Omit<PitchStep, 'state'> & {
  state: PitchContext;
...
export function simulatePitchV3(
  input: PitchContext,
  fixture: Fixture,
  commandId: string,
): PitchV3Step {
  ...
  if (action === 'take') {
    ...
    const ability =
      (pitcher.throwingHand === 'R' ? batter.batting.contactVsRight : batter.batting.contactVsLeft)
        .valueMilli / 120000;
    contactProbability = contactChance(inside, ability, velocityCentiKph, input.count.strikes);
