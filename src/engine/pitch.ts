...
export function selectWeightedIndex(weights: number[], draw: number): number {
    ...
    weights.length > 0 && weights.every((w) => Number.isFinite(w) && w >= 0),
    '選択重みが不正です',
  );
...
export function advanceCount(
  before: Count,
  ruling: Ruling,
): { count: Count; stopReason: StopReason | null } {
  ...
  if (ruling === 'foul')
    return { count: { ...before, strikes: Math.min(2, before.strikes + 1) }, stopReason: null };
...
export function simulatePitch(
  input: PrototypeState,
  fixture: Fixture,
  commandId: string,
): PitchStep {
  validateFixture(fixture);
  validateState(input, fixture);
  id(commandId);
  ...
  const intendedLocation = {
    xMm: [-144, 0, 144][Math.floor(draw() * 3)]!,
    zMm: [600, 800, 1000][Math.floor(draw() * 3)]!,
  };
  const positionWidth =
