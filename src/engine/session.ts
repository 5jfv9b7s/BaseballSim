...
export class PitchSession {
  ...
  dispatch(command: Command): View {
    ...
    if (command.kind === 'reset') {
      integer(command.payload.seed, 1, 0xffffffff, 'seed');
      validateCount(command.payload.count);
    } else ensure(command.payload === null, 'advanceのpayloadはnullです');
    const fingerprint = JSON.stringify([
      command.localWorldId,
      command.expectedStateRevision,
      command.kind,
      command.kind === 'reset'
        ? [command.payload.seed, command.payload.count.balls, command.payload.count.strikes]
        : null,
    ]);
    const previous = this.processed.get(command.commandId);
    if (previous) {
      ensure(previous.fingerprint === fingerprint, '同じ指示IDに異なる内容が届きました');
    ...
    if (command.kind === 'reset')
      next = {
        ...
        state: createInitialState(command.payload.seed, command.payload.count),
        steps: [],
      };
    else {
