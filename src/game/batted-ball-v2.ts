...
export function generateBattedBallV2(
  pitch: PitchRecord,
  fixture: GameFixture,
  defense: TeamSide,
  rngInput: RngState,
...
/** 角度を三角分布で生成。結果の安打／アウトはここで決めない。 */
export function launchAngle(u: number): number {
...
/** 反応後も停止状態から加速する。最高速度へ瞬時には到達しない。 */
export function fielderTravel(seconds: number, topSpeed: number): number {
...
/** 到達距離の逆関数。回収走にも捕球走と同じ加速を使う。 */
export function fielderTravelTime(distance: number, topSpeed: number): number {
  const acceleration = m.fielderAccelerationMetersPerSecond2;
