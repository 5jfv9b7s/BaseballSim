...
import { FIELD_POSITIONS, GAME_MODEL as m } from './model.ts';
...
export function generateBattedBall(
  ...
  const angle = m.angleMinimumDegrees + m.angleSpanDegrees * draw();
  ...
  for (const fielder of fielders) {
    ...
    const reach = Math.max(0, time - reaction) * movement + 1.2;
    const throwSpeed =
  ...
  if (!homeRun && !best) {
    ...
    for (const f of fielders) {
      ...
      const recovery = Math.max(
        ...
        0.5 + gap / (4 + (4 * p.runningSpeed.valueMilli) / 120000),
      );
      const throwSpeed =
        m.throwBaseMetersPerSecond +
        (m.throwAbilityMetersPerSecond * p.armStrength.valueMilli) / 120000;
