...
test('固定seedの既知結果と全状態を再現し、入力を変更しない', () => {
  ...
  assert.equal(step.event.pitch.ruling, 'swingingStrike');
  assert.equal(step.event.pitch.velocityCentiKph, 14659);
  ...
  assert.equal(step.state.rng.fullState.word, 3295023023);
  assert.equal(step.state.rng.drawCount, 8);
});

test('xorshift32の既知ベクトル、完全な状態からの継続', () => {
  let rng = createInitialState(1).rng;
  for (const word of [270369, 67634689, 2647435461]) {
    rng = nextRandom(rng).state;
    assert.equal(rng.fullState.word, word);
  }
...
test('17区画、分割線は右・上、矩形端点は内側', () => {
  ...
  [500, 700, 900].forEach((zMm, row) =>
    [-216, -72, 72].forEach((xMm, col) =>
      assert.equal(classifyZone({ xMm, zMm }, b), expected[row]![col]),
    ),
  );
  ...
  cases.forEach(([xMm, zMm, expectedZone]) =>
    assert.equal(classifyZone({ xMm, zMm }, b), expectedZone),
  );
});
...
test('2ストライク後の通常ファウル、四球・三振・フェアの未解決停止', () => {
  ...
  assert.deepEqual(advanceCount(count, 'swingingStrike'), {
    count,
    stopReason: 'strikeoutPending',
  });
...
test('重みの端点、0重み、負・非有限・全0・空を検査', () => {
  ...
  assert.equal(selectWeightedIndex([0, 1, 1], 0.5), 2);
  assert.equal(selectWeightedIndex([1, 0], 0.9999999), 0);
  for (const weights of [[], [0, 0], [-1, 2], [NaN], [Infinity]])
    assert.throws(() => selectWeightedIndex(weights, 0.5));
  assert.throws(() => selectWeightedIndex([1], 1));
});

test('モデルの版、seed、カウント、能力・ID・球種参照の不正入力を拒否', () => {
  for (const seed of [0, -1, 1.1, NaN, Infinity, 0x100000000])
    assert.throws(() => createInitialState(seed));
  ...
  for (const mutate of [
    (f: typeof FIXTURE) => {
      f.players[0]!.batting.contactVsRight.valueMilli = 120001;
    },
    (f: typeof FIXTURE) => {
      f.players[0]!.batting.contactVsRight.ceilingMilli = 0;
    },
    (f: typeof FIXTURE) => {
      f.players[0]!.playerId = f.players[1]!.playerId;
    },
    (f: typeof FIXTURE) => {
      f.pitches[0]!.playerId = 'missing';
    },
    (f: typeof FIXTURE) => {
      f.pitches.forEach((p) => {
        p.acquisitionProgressMilli = 99999;
      });
    },
    (f: typeof FIXTURE) => {
      f.pitches[0]!.velocity.spreadCentiKph = NaN;
    },
  ]) {
...
test('未習得球を選択せず、両打ちを実際の打席側に変換', () => {
  const f = structuredClone(FIXTURE);
  f.pitches[0]!.acquisitionProgressMilli = 99999;
  f.players[2]!.battingHand = 'S';
  const step = simulatePitch(createInitialState(), f, '1');
  assert.notEqual(step.event.pitch.pitchId, 'pitch-001');
  assert.equal(step.event.pitch.battingSide, 'L');
...
test('制球は位置誤差、再現性は球速幅へ別々に寄与する', () => {
  ...
  control.pitches[0]!.control = { valueMilli: 120000, ceilingMilli: 120000 };
  const stable = structuredClone(FIXTURE);
  stable.pitches[0]!.repeatability = { valueMilli: 120000, ceilingMilli: 120000 };
  const a = simulatePitch(createInitialState(), control, '1');
  const b = simulatePitch(createInitialState(), stable, '1');
...
test('読み取り・再送は乱数や投球数を進めず、衝突・古い指示は原子的に拒否', () => {
  ...
  assert.deepEqual(session.query(), first);
  assert.deepEqual(session.dispatch(command), first);
  first.state.count.balls = 3;
  assert.equal(session.query().state.count.balls, 0);
  ...
  const malformed = {
    ...
    kind: 'reset',
    payload: { seed: 0, count: { balls: 0, strikes: 0 } },
  } as Command;
  assert.throws(() => session.dispatch(malformed));
  assert.deepEqual(session.query(), before);
});
