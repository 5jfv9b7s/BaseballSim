...
test('変更前の実v2保存を読み込み、v3保存後に同じv2へ戻せる', async () => {
  ...
  try {
    ...
    const next = createGame(20260923, undefined, 'game-prototype-v3');
    runToCompletion(next);
    finalizeGame(next);
...
test('変更前に固定した4条件のv3全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v3-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v3');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v3保存を読み込み、v4保存後に同じv3へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v3.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v3-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
    ...
    const storage = new DexieStorageAdapter(db);
    const old = (await storage.loadSnapshot()).record;
    assert.equal(old.state.simulationVersion, 'game-prototype-v3');
    assert.equal(old.state.totalPitches, 284);
    assert.deepEqual(old.state.score, { away: 2, home: 0 });
    const next = createGame(20260923, undefined, 'game-prototype-v4');
    runToCompletion(next);
    finalizeGame(next);
    assert.equal(next.state.simulationVersion, 'game-prototype-v4');
    await storage.commitSnapshot(next, 1);
    assert.deepEqual((await storage.loadSnapshot()).record, next);
    assert.deepEqual((await storage.loadSnapshot(true)).record, old);
  } finally {
...
test('変更前に固定した4条件のv4全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v4-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v4');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v4保存を読み込み、v5保存後に同じv4へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v4.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v4-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
    ...
    const storage = new DexieStorageAdapter(db);
    const old = (await storage.loadSnapshot()).record;
    assert.equal(old.state.simulationVersion, 'game-prototype-v4');
    assert.equal(old.state.totalPitches, 334);
    assert.deepEqual(old.state.score, { away: 0, home: 7 });
    const next = createGame(20260923, undefined, 'game-prototype-v5');
    runToCompletion(next);
    finalizeGame(next);
    assert.equal(next.state.simulationVersion, 'game-prototype-v5');
    await storage.commitSnapshot(next, 1);
    assert.deepEqual((await storage.loadSnapshot()).record, next);
    assert.deepEqual((await storage.loadSnapshot(true)).record, old);
  } finally {
...
test('変更前に固定した4条件のv5全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v5-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v5');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v5保存を読み込み、v6保存後に同じv5へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v5.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v5-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
    ...
    const storage = new DexieStorageAdapter(db);
    const old = (await storage.loadSnapshot()).record;
    assert.equal(old.state.simulationVersion, 'game-prototype-v5');
    assert.equal(old.state.totalPitches, 362);
    assert.deepEqual(old.state.score, { away: 6, home: 2 });
    const next = createGame(20260923, undefined, 'game-prototype-v6');
    runToCompletion(next);
    finalizeGame(next);
    assert.equal(next.state.simulationVersion, 'game-prototype-v6');
    await storage.commitSnapshot(next, 1);
    assert.deepEqual((await storage.loadSnapshot()).record, next);
    assert.deepEqual((await storage.loadSnapshot(true)).record, old);
  } finally {
...
test('変更前に固定した4条件のv6全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v6-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v6');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v6保存を読み込み、v7保存後に同じv6へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v6.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v6-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
...
test('変更前の実v7保存を読み込み、v8保存後に同じv7へ戻せる', async () => {
  ...
  try {
    ...
    const next = createGame(20260923, undefined, 'game-prototype-v8');
    runToCompletion(next);
    finalizeGame(next);
...
test('変更前に固定した4条件のv8全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v8-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v8');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v8保存を読み込み、v9保存後に同じv8へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v8.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v8-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
    ...
    const storage = new DexieStorageAdapter(db);
    const old = (await storage.loadSnapshot()).record;
    assert.equal(old.state.simulationVersion, 'game-prototype-v8');
    assert.equal(old.state.totalPitches, 403);
    assert.deepEqual(old.state.score, { away: 7, home: 9 });
    const next = createGame(20260923, undefined, 'game-prototype-v9');
    runToCompletion(next);
    finalizeGame(next);
    assert.equal(next.state.simulationVersion, 'game-prototype-v9');
    await storage.commitSnapshot(next, 1);
    assert.deepEqual((await storage.loadSnapshot()).record, next);
    assert.deepEqual((await storage.loadSnapshot(true)).record, old);
  } finally {
...
test('変更前に固定した4条件のv9全記録ハッシュを維持する', async () => {
  const digests = JSON.parse(
    readFileSync(new URL('./fixtures/v9-digests.json', import.meta.url), 'utf8'),
  );
  for (const [seed, digest] of Object.entries(digests)) {
    const record = createGame(Number(seed), undefined, 'game-prototype-v9');
    runToCompletion(record);
    finalizeGame(record);
    assert.equal(await sha256(canonicalJson(record)), digest);
    validateCompletedRecord(record);
...
test('変更前の実v9保存を読み込み、v10保存後に同じv9へ戻せる', async () => {
  const frozen = JSON.parse(
    gunzipSync(
      readFileSync(new URL('./fixtures/completed-v9.json.gz', import.meta.url)),
    ).toString(),
  );
  const db = new GameDatabase('legacy-v9-' + crypto.randomUUID());
  try {
    for (const [name, rows] of Object.entries(frozen)) {
      await db.table(name).bulkPut(
        (rows as Record<string, unknown>[]).map((row) => ({
    ...
    const storage = new DexieStorageAdapter(db);
    const old = (await storage.loadSnapshot()).record;
    assert.equal(old.state.simulationVersion, 'game-prototype-v9');
    assert.equal(old.state.totalPitches, 403);
    assert.deepEqual(old.state.score, { away: 7, home: 9 });
    const next = createGame(20260923);
    runToCompletion(next);
    finalizeGame(next);
    assert.equal(next.state.simulationVersion, 'game-prototype-v10');
    await storage.commitSnapshot(next, 1);
    assert.deepEqual((await storage.loadSnapshot()).record, next);
    assert.deepEqual((await storage.loadSnapshot(true)).record, old);
  } finally {
